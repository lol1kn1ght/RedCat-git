...
class Command {
  ...
  async execute(bot, message, args, mongo) {
    ...
    let members = await message.guild.members.fetch({
      id: users_data.map(val => val.login)
    ...
    for (let user of users_data) {
      ...
      if (member.id === message.author.id) author_pos = user_pos;
    ...
    for (let page of pages) {
      let page_embed = new Discord.MessageEmbed()
        ...
        .setFooter(
          `Страница ${embed_page++} из ${
            pages.length
          } - Ваша позиция ${author_pos || "Нету :с"}`
        );
    ...
    if (!embeds_pages[0]) {
      let page_embed = new Discord.MessageEmbed()
        ...
        .setColor(f.config.defColor)
        .setFooter(
          `Страница ${embed_page++} из ${pages.length ||
            1} - Ваша позиция: ${author_pos || "Нету :с"}`
        );
      embeds_pages.push(page_embed);
    }
