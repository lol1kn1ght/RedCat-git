...
class Command {
  ...
  async execute(bot, message, args, mongo) {
    ...
    let last_ids = settings.items_ids || []; // Достаем из настроек массив с исторей айди предметов
    ...
    if (settings_data[0]) {
      ...
      settings_db.insertOne({ guildID: message.guild.id, items_ids: last_ids });
    }
  ...
  #getOptions() {
    return {
      ...
      usage: `create-item [название предмета] [цена]`,
      enabled: true,
      type: "Магазин",
  ...
  #getSlashOptions() {
    return {
      ...
      options: [
        {
          name: "item-name",
          description: "имя нового предмета",
          type: 3,
          required: true,
        },
        {
          name: "item-cost",
          description: "цена нового предмета",
          type: 4,
          required: true,
        },
      ],
