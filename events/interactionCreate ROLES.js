  ...
  async execute(bot, mongo, interaction) {
    ...
    if (
      ...
      if (interaction_member.roles.cache.has("689152734469161015")) {
        ...
        interaction.reply({
          content: `Выдана роль платформы ${role}. **ВНИМАНИЕ! Для поиска игроков пользуйтесь <#919520554493562910>. __Попытки найти пати в непредназначенных для этого каналах наказуемы!__**`,
          ephemeral: true
        });
        ...
        const role_platform = new MessageActionRow().addComponents(
          new MessageButton()
            ...
            .setLabel(
              `ПК (${(
                interaction.guild.roles.cache.get(`626800044884099082`).members
                  .size / 1000
              ).toFixed(1)}k)`
            )
          ...
          new MessageButton()
            ...
            .setLabel(
              `XBOX (${(
                interaction.guild.roles.cache.get(`598163957081178126`).members
                  .size / 1000
              ).toFixed(1)}k)`
            )
          ...
          new MessageButton()
            ...
            .setLabel(
              `PlayStation (${(
                interaction.guild.roles.cache.get(`598164183586177064`).members
                  .size / 1000
              ).toFixed(1)}k)`
            )
